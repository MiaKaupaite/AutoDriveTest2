...
void Drive::straightAutonMethod(double speed, double distance) {
	if (Robot::drive->getLeftEncPos() < distance && Robot::drive->getLeftEncPos() < distance){
		if (Robot::drive->getGAngle() > 2) {
			//veering right
			Robot::drive->leftMotor1->Set(-(speed-(getGAngle()*0.01)));
			Robot::drive->rightMotor1->Set(-(speed+(getGAngle()*0.01)));
			Robot::drive->leftMotor2->Set(-(speed-(getGAngle()*0.01)));
			Robot::drive->rightMotor2->Set(-(speed+(getGAngle()*0.01)));
